...
inline static std::string NormalizePath(const std::string& in) {
    // Trim any leading slashes to match keys like "app.html" generated by embed_files.py
    size_t i = 0;
...
bool EmbeddedFileSystem::FileExists(const String& file_path)
{
    ...
    auto key = NormalizePath(ToString(file_path));
    return embedded_files.find(key) != embedded_files.end();
}
...
RefPtr<Buffer> EmbeddedFileSystem::OpenFile(const String& file_path)
{
    auto& embedded_files = GetEmbeddedFiles();
    auto key = NormalizePath(ToString(file_path));
    auto it = embedded_files.find(key);
